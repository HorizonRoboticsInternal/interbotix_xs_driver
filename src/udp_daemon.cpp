...
namespace horizon
{
  ...
  path += username; path += "/interbotix_xs_driver";
  std::string filepath_motor_configs, filepath_mode_configs;
  filepath_motor_configs = path + "/wx250s_motor_config.yaml";
  filepath_mode_configs = path + "/mode_configs.yaml";
  bool write_eeprom_on_startup = true;
  std::string logging_level = "DEBUG";

  arm_low_ = std::make_unique<InterbotixDriverXS>(
      filepath_motor_configs, filepath_mode_configs, write_eeprom_on_startup, logging_level);
